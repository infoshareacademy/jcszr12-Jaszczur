...
internal static class LoggingExtensions
{
    ...
    {
        string scopeMessageFormat = $"Service: {{Service}} Method: {{Method}} Request: {(destructureRequest ? "{@Request}" : "{Request}")}";
        var scope = logger.BeginScope(scopeMessageFormat, typeof(TService).Name, methodName, request);

        string logMessageFormat = $"Service method call - Service: {{Service}} Method: {{Method}} Request: {(destructureRequest ? "{@Request}" : "{Request}")}";
    ...
    {
        string logMessageFormat = $"Returning Response: {(destructureResponse ? "{@Response}" : "{Response}")}";
